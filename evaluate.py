...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Score a submission against locally available competition labels.')
    parser.add_argument('submission_path', type=Path, help='CSV with ts_code, trade_date, pred')
    parser.add_argument('--data-dir', type=Path, required=True,
                        help='directory containing 测试集_Y.csv and 测试集_X.csv')
    args = parser.parse_args()
    result = evaluate(str(args.submission_path), str(args.data_dir))
    print('\n===== 评分结果 =====')
