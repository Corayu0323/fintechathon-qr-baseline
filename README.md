...
准备过程会在 `data/prepared/` 生成按年份分区的数据，在 `data/features/factor_v0.2/` 生成按版本分区的因子矩阵和 catalog。这些文件已被 `.gitignore` 排除。公式与因子元数据分别维护在 `scripts/factors/` 和 `specs/factor_set_v0.2.json`；同一版本目录不会被重建覆盖，公式变化需提升版本。
...
.venv/bin/python scripts/factor_diagnostics.py
```
...
研究看板是可离线打开的固定版式，包含 D 综合分候选图、Rank IC/Top 组超额/稳定性分项表、D/H/S 阶段状态及可筛选运行记录。每次新增实验后运行 `scripts/build_dashboard.py` 刷新 `outputs/dashboard/index.html`；也可以直接在浏览器打开该文件。
...
- `scripts/prepare_data.py`：保留原始行并生成标签成熟、行情有效等数据标记。
- `scripts/build_factors.py`：读取版本化因子清单并调度公式函数，按因子集版本原子生成矩阵，不读取标签。
- `scripts/factors/`：价格形态、收益和成交活跃度公式；每个公式函数独立、无状态。
- `scripts/factor_preprocessing.py`：独立实现 identity、训练期 z-score、每日截面 z-score 和截面 rank，并保存/复用训练期 scaler 状态。
- `specs/factor_set_v0.2.json`：机器可读的因子清单、输入字段和公式元数据。
- `scripts/validate_factors.py`：核对 key、缺失掩码、数值 parity、未来扰动和分区 warm-up。
- `scripts/factor_diagnostics.py`：在 D 段计算单因子覆盖率、日度 IC/RankIC 汇总和因子间相关性，并记录到 Qlib Recorder；不以单因子指标替代官方模型评分，也不允许把诊断窗口延伸到 H/S。
- `scripts/qlib_bridge.py`：对齐因子、标签和辅助字段；训练按成熟时点过滤，测试不加载标签。
- `scripts/run_fixed_baseline.py`：D/H/S 阶段配置、拟合、预测、回填和输出。
...
- `scripts/models/`：LightGBM 选轮数与最终拟合、Qlib Ridge 适配、透明信号预测；runner 负责阶段编排及数据/标签时钟。
- `scripts/models/config.py`：加载并校验模型配方和实验方案。
- `scripts/validate_model_config.py`：检查配方/阶段默认值，并对模型适配器做轻量 smoke test。
- `scripts/experiment_tracking.py`：SQLite 实验库初始化与 Qlib Recorder 工具。
- `scripts/import_existing_runs.py`：将上线记录前已完成的 D/H/S 结果幂等导入实验库。
- `scripts/build_dashboard.py`：从实验记录库生成固定版式、可筛选的本地 HTML 看板。
- `scripts/official_score.py`：本地评分适配器。
- `scripts/validate_pipeline.py`：对照题给评分器、核验阶段边界、预测键和提交格式。
